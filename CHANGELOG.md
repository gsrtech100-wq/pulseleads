...
All notable changes to PulseLeads are documented here. This product evolved from
...
- Substack selected as **Connector #1** (reuses the proven StackPulse `SubstackFetcher`).
...
- **First-run experience:** input prefill examples (`keywords`, `newsletterUrls`) + plain-language help.
...
- Repository organized as a production commercial project (LICENSE, CHANGELOG, RELEASE_NOTES,
  architecture doc, examples, sample output).
